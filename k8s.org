...
| Get deployment yaml                  | =kubectl -n sammy-websites get deployment mysql -o yaml=                                  |
...
| List all container images            | kubectl get pods --all-namespaces -o jsonpath="{..image}" \| sort \| uniq -c              |
| kubeconfig skip tls verification     | https://github.com/terraform-providers/terraform-provider-kubernetes/issues/189           |
...
| Name                             | Command                                                           |
|----------------------------------+-------------------------------------------------------------------|
| Filter pods by label             | =kubectl get pods -l owner=sammy=                                 |
| Manually add label to a pod      | =kubectl label pods dummy-input owner=sammy=                      |
| Remove label                     | =kubectl label pods dummy-input owner-=                           |
| Manually add annonation to a pod | =kubectl annotate pods dummy-input my-url=https://xyz           = |
...
| Get secret                       | =kubectl get secret sammy-cluster-kubeconfig=                           |
| Get a specific field of a secret | kubectl get secret sammy-cluster-kubeconfig -o jsonpath="{.data.value}" |
